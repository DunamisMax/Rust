...
2. **Terminal UI & Interaction**
   - Integrate **tui** (`tui` crate) for all terminal-based UIs (menus, text styling, layout) to craft a modern TUI experience.
...
6. **Initial Structure**
   - **Begin** your generated CLI app by clearing the screen, printing a **welcome banner**, and greeting the user—using `tui` + `crossterm`.
...
2. **Adherence to Guidelines**: When generating **CLI applications**, you **must** employ **Tokio** (async runtime), **tui** + **crossterm** (TUI & terminal control), **Clap** (CLI parsing), and **cross-platform line endings**.
